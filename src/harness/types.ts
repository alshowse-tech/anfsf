...
export interface CanaryOptions {
  ...
  stages?: number[]; // e.g., [0.01, 0.05, 0.2, 0.5, 1.0]
  ...
  significanceCheck?: {
    enabled: boolean;
    threshold: number; // p-value threshold
    minSampleSize: number;
  };
}
...
export interface AgentHarnessConfig {
  ...
  defaultSignificanceThreshold?: number; // p < 0.05
...
export function isTestResult(obj: any): obj is TestResult {
  return (
    ...
    ['pending', 'running', 'passed', 'failed', 'cancelled'].includes(obj.status)
  );
}

export function isDeploymentResult(obj: any): obj is DeploymentResult {
  return (
    typeof obj === 'object' &&
...
export function isOwnershipCheckResult(obj: any): obj is OwnershipCheckResult {
  return (
    typeof obj === 'object' &&
