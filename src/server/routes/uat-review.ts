...
export function registerUATRoutes(app: FastifyInstance): void {
  app.post("/api/v1/uat/review", async (req) => {
    var body=req.body as {projectId:string;reviewer:string;decision:string;comments?:string};
    ...
    var review=createReview(body.projectId,body.reviewer,body.decision as any,body.comments||"");
    return {status:"ok",review};
  });

  app.get("/api/v1/uat/reviews", async (req) => {
    var query=req.query as {projectId?:string};
    var reviews=getReviews(query.projectId);
    return {status:"ok",reviews,total:reviews.length};
  });

  app.get("/api/v1/uat/reviews/:id", async (req) => {
    var params=req.params as {id:string};
    var review=getReview(params.id);
