...
describe("RecoveryEngine", () => {
  ...
  it("introspection engine optional and works when provided", async () => {
    const ie = new IntrospectionEngine({ sourceDirs: ["src"], apiKey: "test" });
    const s = new InMemoryCheckpointStore();
    const m = new CheckpointManager(s);
    const engine = new RecoveryEngine(m, undefined, ie);
    const machine = new PipelineStateMachine("intro-test", "stage5_done");
    engine.register(machine);
    await expect(machine.transition("stage5_evolving")).resolves.not.toThrow();
});
