...
// 自动修复函数
// ============================================================================
async function attemptFix(status) {
    // 如果样式未加载，尝试重新注入
    if (!status.stylesLoaded) {
...
function showErrorIndicator(errors) {
...
async function readinessGate() {
    ...
    while(retries < READINESS_CONFIG.maxRetries){
        try {
            ...
            retries++;
            // 等待一段时间后重试
            await new Promise((resolve)=>setTimeout(resolve, READINESS_CONFIG.probeInterval));
        } catch (error) {
