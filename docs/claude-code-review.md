...
Phase 4 ????? + ????? + API ???????Claude Code ?????

### ????

| ?? | ?? | ?? | ?? |
|------|------|------|------|
| KnowledgeBridge | `src/pipeline/knowledge-bridge.ts` | 2/2 | ?? CompileLearningDB + ComponentMiner ? KnowledgeBase?? LLM ?? |
| MetricsCollector | `src/pipeline/metrics-collector.ts` | 5/5 | ?????? + P95 / ???? / Token ???? |
| Knowledge Routes | `src/server/routes/knowledge.ts` | ? | 4 ??: compile-patterns / component-patterns / metrics / bottlenecks |

### ????

| ?? | ?? |
|------|------|
| `code-generation-loop.ts` | ? `getKnowledgeInjection()` ?? `getComponentMiner()` ?? |
| `compile-learning-db.ts` | ?? `clear()` ???????? |
| `evolution-runner.ts` | MetricsCollector ???????????? |
| `metrics-collector.ts` | ?? `getAllStageSummaries()` |
| `server/index.ts` | ?? knowledge routes |
| `INDEX.md` | ??????? |
...
| # | ??? | ?? | ?? |
|---|--------|------|------|
| 1 | LOW | `compile-validator.test.ts` | Windows EPERM symlink ? ?????? |
| 2 | LOW | `product-pipeline.test.ts` | LLM mock ???? parser ? ?? mock ?? |
