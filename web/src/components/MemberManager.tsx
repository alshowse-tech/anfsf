...
export default function MemberManager() {
  ...
  const addMember = async()=>{
    ...
    try{
      ...
      const d=await r.json(); if(d.members){setMembers(d.members);setNewUserId("");setMessage("Member added");}else{setMessage("Error: "+(d.error?.message||"Failed"));}
  ...
  return (
    <div className="space-y-3">
      <h3 className="text-sm font-medium text-gray-700">Member Management</h3>
      <div><label className="block text-xs text-gray-500 mb-1">Tenant</label>
      ...
      <div className="space-y-2">
        {members.map(m=>(
          <div key={m.userId} className="flex items-center justify-between border rounded-lg p-2">
            ...
            <button onClick={()=>removeMember(m.userId)} className="px-2 py-1 text-xs bg-red-100 text-red-700 rounded hover:bg-red-200">Remove</button>
          </div>
        ))}
        {members.length===0&&<p className="text-xs text-gray-500 py-4 text-center">No members</p>}
      </div>
      <div className="flex gap-2 items-end">
        <div className="flex-1"><label className="block text-xs text-gray-500 mb-1">User ID</label>
          <input value={newUserId} onChange={e=>setNewUserId(e.target.value)} className="w-full px-3 py-1.5 border border-gray-300 rounded-md text-sm" placeholder="username" /></div>
        ...
        <button onClick={addMember} disabled={!newUserId} className="px-3 py-1.5 bg-blue-600 text-white rounded-md text-sm hover:bg-blue-700 disabled:opacity-50">Add</button>
      </div>
